...
export default function NewServicePage() {
  ...
  const onSubmit = async (data: ServiceFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(result.error || "Chyba při vytváření služby")
        return
      }
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Přidat novou službu</h1>
        <p className="text-muted-foreground">
          Vytvořte novou službu pro vaše zákazníky
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={isLoading} size="lg">
            {isLoading ? "Vytváří se..." : "Vytvořit službu"}
          </Button>
          <Button
            type="button"
            variant="outline"
